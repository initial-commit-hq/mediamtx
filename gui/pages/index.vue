<template>
  <div class="page-wrap">
    <AppHeader :status-text="statusText" :status-class="statusClass" @connect="connect">
      <template #actions>
        ...
        <div v-if="connected" class="layout-picker">
          <button
            ...
            class="layout-btn" :class="{ active: cols === c }"
            @click="setLayout(c)"
          >{{ c === 1 ? '1' : c === 2 ? '4' : '9' }}</button>
        </div>
      </template>
    </AppHeader>
    ...
    <div v-if="allStreams.length > PAGE_SIZE" class="stream-bar">
      ...
      <span
        v-for="name in allStreams" :key="name"
        class="stream-tag" :class="{ active: selectedNames.includes(name) }"
    ...
    <div v-if="pageCount > 1" class="pagination">
      ...
      <button
        ...
        class="page-btn" :class="{ active: page === i - 1 }"
        @click="page = i - 1"
      >{{ i }}</button>
...
const displayNames  = computed(() => selectedNames.value.length ? selectedNames.value : allStreams.value)
const pageCount     = computed(() => Math.ceil(displayNames.value.length / perPage.value))
...
function toggleStream(name: string) {
  ...
  if (idx === -1) {
    if (selectedNames.value.length >= PAGE_SIZE) selectedNames.value.shift()
...
function prevPage() { if (page.value > 0) { page.value--; renderKey.value++ } }
function nextPage()  { if (page.value < pageCount.value - 1) { page.value++; renderKey.value++ } }
...
.bar-label { font-size: 12px; color: var(--text-dim); margin-right: 4px; }
.stream-tag {
...
.stream-tag.active { border-color: var(--accent); background: rgba(59, 130, 246, 0.15); color: var(--text); }
...
.layout-btn.active { background: rgba(59, 130, 246, 0.2); border-color: var(--accent); color: var(--text); }
